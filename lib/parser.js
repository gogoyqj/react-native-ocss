            ...
            if (trimLine.indexOf("$") === 0) {
                ...
                if (inClass) { // 解析样式
                    ...
                    trimLine.map(function(item) {
                        ...
                        if (mt && mt[0] && mt[1]) {
                            ...
                            if (rule === "background") {
                                ...
                                console.log("【Log】：自动替换background" + " 为 backgroundColor" +  "，文件：" + context.file + "，className：" + inClass)
                            }
                            // 检测属性
                            if (!(rule in allProp)) {
                                reportError("不支持的属性：" + rule + "【" + mt[0] + "】" +  "，文件：" + context.file + "，className：" + inClass)
                            }
                            // do something
                            ...
                            v = mt[1].replace(/\$[a-z0-9_\.]+/gi, function(v) {
                                ...
                                if (res === undefine) {
                                    reportError("未能查找到变量：" + v +  "，文件：" + context.file + "，className：" + inClass)
                                } else {
                                    // 处理字符串
                                ...
                                if (transforms) {
                                    ...
                                    transforms.forEach(function(transform) {
                                        ...
                                        if (!(transformPro in _transformInAllProp)) {
                                            reportError("不支持的属性：" + rule + "【" + mt[0] + " " + transform + "】" +  "，文件：" + context.file + "，className：" + inClass)
                                        } else if (_transformInAllProp[transformPro](value) === false) {
                                            reportError("赋给属性" +  rule + "【" + mt[0] + "】的值不对：" + v + "【" + mt[1] + "】" +  "，文件：" + context.file + "，className：" + inClass)
                                        } else {
                                            newObj.push(obj)
                                        }
                                    })
                            ...
                            reportError("未能解析的样式：" + item + "，文件：" + context.file + "，className：" + inClass)
                        }
                    })
                } else { // 解析class
                    trimLine = trimLine.split(",")
                    ...
                    trimLine.map(function(item) {
                        ...
                        if (!item) return
                        if (item.indexOf(" ") !== -1) {
                            reportError("不支持层叠：" + item + "，文件：" + context.file + "，className：" + inClass)
                        } else {
