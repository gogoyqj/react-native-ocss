...
var stringParser = exports.stringParser = function (content, context, parContext, globalVars, inClass, isSass) {
    if (isSass) content = sass.renderSync({data: content, includePaths:  [context && context.file ? path.dirname(context.file) : "./"]}).css.toString();
    var stack = [];
                    ...
                    trimLine.map(function(item) {
                        ...
                        if (mixinPos > -1) {
                            ...
                            if (mt && mt[0] && mt[1]) {
                                ...
                                if (rule === "background") {
                                ...
                                } else if (rule in {"borderRadius": 1, "padding": 1, "margin": 1}) {
                                    var pts = mt[1].split(/[ ]+/g)
                                    if (pts.length > 1) {
                                    ...
                                    mt[1] = mt[1].trim().replace(/[ ]{2,}/g, " ")
                                } else if (rule == "boxShadow") {
                                    pts = v.replace(/(outset|inset)$/g, "").split(",")[0].trim().split(/[ ]+/g)
                                    // 0 1 2    3      4     5
