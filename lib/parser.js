...
function forMixin(item, mixinPos, context, globalVars) {
    ...
    mixin = mixin.split(" ")
    console.log("【Log】：调用mixin " + item + "，文件：" + context.file)
    if (mixinPos === 0) {
        ...
        if (typeof mixinFunc !== "function") {
            reportError("调用了未定义的mixin：" + mixinName +  "，文件：" + context.file + "，行：" + item)
        } else {
            var mixinProp = mixinFunc.apply(null, mixinArgs)
            // 必须是对象
            if (typeof mixinProp !== "object" || mixinProp.join) {
                mixinProp = {}
                console.log("【warning】mixin：" + mixinName + "返回值类型错误")
            }
            return mixinProp
        }
    } else {
        reportError("在错误的位置调用了mixin：" + mixin[0] +  "，文件：" + context.file + "，行：" + item)
    }
    return {}
}
                                ...
                                arg.forEach(function(a, index) {
                                    ...
                                    if (index === 1) {
                                        if (_a.match(/^[0-9\.]+$/g)) {
                                            ...
                                            if (_a === undefine) {
                                                console.log("【warning】mixin " + defineMixinName + "的参数" + arg[0] + "指定的默认值为" + undefine  + "，文件：" + context.file + "，行：" + trimLine)
                                            }
                                        }
                                    } else if (!index && !_a) {
                ...
                if (!mixinStack.length) {
                    ...
                    */
                    ...
                    tmp.forEach(function (newLine, i) {
                        ...
                        if (innerClass) {
                            if (toKeep === 2) {
                                ...
                                console.log("【warning】过滤掉mixin " + defineMixinName + "内：" + newLine +  "，文件：" + context.file + "，行：" + newLine)
                            }
                            if (newLine === "{") {
                                innerStack.push("{")
                            } else if (newLine === "}") {
                    ...
                    context.vars[defineMixinName] = function () {
                        ...
                        var mixinContext = stringParser(newArr, {vars: vars}, context, globalVars, toKeep === 1)
                ...
                if (inClass) { // 解析样式
                        ...
                        if (mixinPos > -1) {
                            ...
                            if (mt && mt[0] && mt[1]) {
                                ...
                                if (rule === "background") {
                                    ...
                                    console.log("【Log】：自动替换background" + " 为 backgroundColor" +  "，文件：" + context.file + "，className：" + inClass + "，行：" + item)
                                }
                                // 检测属性
                                if (!(rule in allProp)) {
                                    reportError("不支持的属性：" + rule + "【" + mt[0] + "】" +  "，文件：" + context.file + "，className：" + inClass + "，行：" + item)
                                }
                                // 转化值
                                v = mt[1].replace(/\$[a-z0-9_\.]+/gi, function(v) {
                                    ...
                                    if (res === undefine) {
                                        reportError("未能查找到变量：" + v +  "，文件：" + context.file + "，className：" + inClass + "，行：" + item)
                                    } else {
                                        // 处理字符串
                                ...
                                if (rule === "transform") {
                                    ...
                                    if (transforms) {
                                        ...
                                        transforms.forEach(function(transform) {
                                            ...
                                            if (!(transformPro in _transformInAllProp)) {
                                                reportError("不支持的属性：" + rule + "【" + mt[0] + " " + transform + "】" +  "，文件：" + context.file + "，className：" + inClass + "，行：" + item)
                                            } else if (_transformInAllProp[transformPro](value) === false) {
                                                reportError("赋给属性" +  rule + "【" + mt[0] + "】的值不对：" + v + "【" + mt[1] + "】" +  "，文件：" + context.file + "，className：" + inClass + "，行：" + item)
                                            } else {
                                                newObj.push(obj)
                                            }
                                        })
                                    ...
                                    tempClass[rule] = v;
                                    // 值不对
                                    if ((rule in allProp) && allProp[rule](v) === false) {
                                        reportError("赋给属性" +  rule + "【" + mt[0] + "】的值不对：" + v + "【" + mt[1] + "】" +  "，文件：" + context.file + "，className：" + inClass + "，行：" + item)
                                    }
                                }
                            } else {
                                reportError("未能解析的样式：" + item + "，文件：" + context.file + "，className：" + inClass + "，行：" + item)
                            }
                        }
                    ...
                    if (trimLine.indexOf("@") === 0) {
                        ...
                        trimLine.map(function(item) {
                            ...
                            if (!item) return
                            if (item.indexOf(" ") !== -1) {
                                reportError("不支持层叠：" + item + "，文件：" + context.file + "，className：" + inClass + "，行：" + item)
                            } else {
