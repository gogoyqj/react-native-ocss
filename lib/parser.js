                ...
                if (inClass) { // 解析样式
                    ...
                    trimLine.map(function(item) {
                        ...
                        if (mixinPos > -1) {
                            ...
                            setV(tempClass, forMixin(item, mixinPos, context, globalVars)["@main"] || {})
                        } else {
                            ...
                            if (mt && mt[0] && mt[1]) {
                                    ...
                                    if (pts.length > 1) {
                                        ...
                                        if (rule === "borderRadius") {
                                            ...
                                            pos.forEach(function (p, index) {
                                                ...
                                                setV(tempClass, rule, null, "remove")
                                ...
                                if (newContent) {
                                    ...
                                    setV(tempClass, bdObj["context"]["@main"])
                                    newContent = ""
                                    return
                                }
                                // transform 需要特殊处理一下
                                if (rule === "transform") {
                                    ...
                                    if (transforms) {
                                        ...
                                        setV(tempClass, "transform", newObj)
                                    }
                                } else {
                                    try {
                    ...
                    if (trimLine.indexOf("@") === 0) {
                        ...
                        trimLine = trimLine.split(",")
                        tempClass = tempClass || []
                        trimLine.map(function(item, index) {
                            item = item.trim()
                            if (!item) return
                            if (item.indexOf(" ") !== -1) {
                                ...
                                if (context.context[item]) {
                                    tempClass[index] = context.context[item]
                                } else {
                                    context.context[item] = tempClass[index] = tempClass[index] || {}
                                }
                            }
                        })
                    }
...
function setV(arr, k, v, action) {
    arr = arr.join ? arr : [arr]
