...
var stringParser = exports.stringParser = function (content, context, parContext, globalVars) {
    ...
    // 存储当前的class
    var tempClass, lastLine = "没有指定className，就出现了{", inClass, inMixin, defineMixinName
    // 逐行解析
        ...
        if (defineMixinPos > -1 || inMixin) {
            ...
            if (!inMixin) {
                ...
                if (!defineMixinName) {
                    ...
                    var defineArgs = mainParts.replace(/^[^\s]+/g, "").trim()
                    if (defineArgs) {
                ...
                if (inClass) { // 解析样式
                    ...
                    trimLine.map(function(item) {
                        ...
                        if (mixinPos > -1) {
                            ...
                            mix(tempClass, forMixin(item, mixinPos, context, globalVars))
                        } else {
